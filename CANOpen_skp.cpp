...
#define TX_GPIO_NUM                     GPIO_NUM_10
#define RX_GPIO_NUM                     GPIO_NUM_11
#define STBY_GPIO_NUM					          GPIO_NUM_5
...
static void CO_main(void* arg) {
	...
	while (reset != CO_RESET_APP) {
		...
		while (reset == CO_RESET_NOT) {
			...
			uint8_t LED_red = CO_LED_RED(CO->LEDs, CO_LED_CANopen);
			uint8_t LED_green = CO_LED_GREEN(CO->LEDs, CO_LED_CANopen);
			if(LED_red == 0)
				digitalWrite(LED_CO_ERR,LOW);
			else digitalWrite(LED_CO_ERR,HIGH);
			
			if(LED_green == 0)
				digitalWrite(LED_CO_RUN,LOW);
			else digitalWrite(LED_CO_RUN,HIGH);
...
void CO_loop(void)
{
	...
	{
		pastTime = currentTime;
		OD_RAM.x6001_counter = count++;

		int analogValue = analogRead(POT_P1);
		Serial.printf("ADC P1 analog value = %d\n", analogValue);
		OD_RAM.x6401_readAnalogueInput16Bit[0] = analogValue;

		analogValue = analogRead(POT_P2);
		Serial.printf("ADC P2 analog value = %d\n", analogValue);
		OD_RAM.x6401_readAnalogueInput16Bit[1] = analogValue;

		OD_RAM.x6000_readInput8Bit[0] = (digitalRead(SW2)<<1) | (digitalRead(SW1));  // Read switches

		Serial.printf("0x6000 %x\n",OD_RAM.x6000_readInput8Bit[0]);

		if(OD_RAM.x6200_writeOutput8Bit[0] & 1<<0)		// Bit 0
		{
		...
		if(OD_RAM.x6200_writeOutput8Bit[0] & 1<<1)		// Bit 1
		{
	...
	if (NMT_opState != CO->NMT->operatingState)
	{
		...
		ESP_LOGI(CANOPEN_TAG, "Operating state: %d", CO->NMT->operatingState);
	}
